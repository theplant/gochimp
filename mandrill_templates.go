...
func (a *MandrillAPI) TemplateAdd(name string, code string, text string, publish bool, subject string) (Template, error) {
	if name == "" {
...
// can error with one of the following: Unknown_Template, Invalid_Key, ValidationError, GeneralError
func (a *MandrillAPI) TemplateUpdate(name string, code string, text string, publish bool, subject string) (Template, error) {
	if name == "" {
		return Template{}, errors.New("name cannot be blank")
	}
	if code == "" {
